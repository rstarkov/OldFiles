...
public class OldFilesTests
{
    private class tItem : IOldItem
    {
        ...
        public State State
        {
            ...
            set
            {
                if (value == _state)
                    return;
                Assert.Equal(State.Undecided, _state);
                _state = value;
                Modified = true;
            }
        }
        ...
        public tItem() { }
        public tItem(double age, double spacing, State state = State.Undecided) { Age = age; Spacing = spacing; _state = state; }
    ...
    private static void assertTestItemSpacingCorrect(List<tItem> items)
    {
        ...
        {
            Assert.NotEqual(State.Undecided, items[i].State);
            if (items[i].Modified)
            {
                ...
                Assert.Equal(expectedState, items[i].State);
            }
        }
    }
    ...
    private static void test(IEnumerable<tItem> items, params State[] expectedStates)
    {
        ...
        foreach (var item in items)
        {
            Assert.Equal(expectedStates[i], item.State);
            i++;
        }
